...
也沒提到 agent API 實際有多慢：`POST /media` 每張約 50 秒，而同一批檔案在網頁上傳只要 0.7 秒，
所以整包素材上架的正解是「圖在網頁傳、欄位用一次 REST 補」。連 502 的語意都要分操作看——
`PUT` 是整包覆蓋所以重試無害，`POST /media` 收到 502 卻可能已經寫進去了，直接重試會產生重複。
